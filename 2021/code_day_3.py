...
You need to use the binary numbers in the diagnostic report to generate two new binary numbers (called the gamma rate and the epsilon rate). The power consumption can then be found by multiplying the 
...
The epsilon rate is calculated in a similar way; rather than use the most common bit, the least common bit from each position is used. So, the epsilon rate is 01001, or 9 in decimal. Multiplying the
gamma rate (22) by the epsilon rate (9) produces the power consumption, 198.

Use the binary numbers in your diagnostic report to calculate the gamma rate and epsilon rate, then multiply them together. What is the power consumption of the submarine? (Be sure to represent 
...
with open('D:/Users/jcaddick/aoc/aoc/2021/input_day_3.txt') as f:
    str_data = f.readlines()
    str_data = [d.strip() for d in str_data]
int_data = [list(d) for d in str_data]
int_data = [[int(i) for i in d] for d in int_data]
int_data = np.array(int_data)
int_data_t = int_data.T
epsilon = ''.join(['1' if sum(d) > 500 else '0' for d in int_data_t])
epsilon = ''.join(['1' if sum(d) > 500 else '0' for d in int_data_t])
gamma = ''.join(['1' if bit == '0' else '0' for bit in list(epsilon)])
epsilon = int(epsilon, 2)
gamma = int(gamma, 2)

print(f'the answer to part 1 is {epsilon * gamma}')
...
import pandas as pd
sep_data = [','.join(i) for i in str_data]
binary = pd.DataFrame({'sep_binary': sep_data})
oxygen = binary.sep_binary.str.split(',', expand=True).astype(int)
carbon = binary.sep_binary.str.split(',', expand=True).astype(int)
for col in oxygen:
    if len(oxygen) == 1:
        break
    elif oxygen[col].sum() >= math.ceil(len(oxygen) / 2):
        oxygen = oxygen[oxygen[col] == 1]
    else:
        oxygen = oxygen[oxygen[col] == 0]
for col in carbon:
    if len(carbon) == 1:
        break    
    if carbon[col].sum() >= math.ceil(len(carbon) / 2):
        carbon = carbon[carbon[col] == 0]
    else:
        carbon = carbon[carbon[col] == 1]
oxygen = int(oxygen.astype(str).agg(''.join, axis=1).values[0], 2)
carbon = int(carbon.astype(str).agg(''.join, axis=1).values[0], 2)
print(f'the answer to part 2 is {oxygen * carbon}')