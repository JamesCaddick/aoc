...
Although you know nothing about this specific species of lanternfish, you make some guesses about their attributes. Surely, each lanternfish 
...
def more_lantern_fish(days, counts):
    results = {n: i for n, i in zip(range(0, 9), counts)}
    for day in range(days):
        zeros = results[0]
        for i in range(6):
            results[i] = results[i + 1]
        results[6] = results[7] + zeros
        results[7] = results[8]
        results[8] = zeros
    return results
            
with open('D:/Users/jcaddick/aoc/aoc/2021/input_day_6.txt') as f:
    data = f.read().split(',')
data = np.array([int(d) for d in data])
counts = [np.count_nonzero(data == n) for n in range(0, 9)]
print(f'the answer to part 1 is {sum(more_lantern_fish(80, counts).values())}')
print(f'the answer to part 2 is {sum(more_lantern_fish(256, counts).values())}')
